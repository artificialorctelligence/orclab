...
root. The schema is Microsoft's, not Unity's: `RuleSet Name … ToolsVersion`, `Rules AnalyzerId …
RuleNamespace`, `Rule Id … Action` are the elements of Visual Studio's *How to: Create a custom
rule set* page (Unity's page links it, and its own ErrorProne.NET example uses the same three
with `ToolsVersion="10.0"`; both read live 2026-09-19):
