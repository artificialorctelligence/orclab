...
- 512 px tiles: `tileDimension: 512, zoomOffset: -1` — for overlay servers that offer 512 px
  tiles, not for OSM.
...
`geolocator` **14.0.3** (2026-06-12) is the current position plugin. A radius around the user
is a `CircleLayer` with `useRadiusInMeter: true`; zoom-to-radius is arithmetic on the map camera
(`fitCamera` to the circle's bounds), no library.
...
- `flutter_map_tile_caching`, `geolocator`: `https://pub.dev/api/packages/<name>`
