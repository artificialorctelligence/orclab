...
| Path | What |
|---|---|
| `pubspec.yaml` | Dependencies, assets, and **`version: 1.0.0+1`** — the single source of the app's version. `1.0.0` becomes Android `versionName` and iOS `CFBundleShortVersionString`; `+1` becomes `versionCode` and `CFBundleVersion`. Both stores require the build number to increase on every upload. `/orc-version` edits this file (since 2026-09-22, BACKLOG #6): it rewrites the version and **always increments `+N`**, including when the version itself is unchanged — the re-upload-after-rejection case still needs a build number the store has never seen. |
...
| Version code increases every upload | `pubspec.yaml` `version: x.y.z+N` — `/orc-version` bumps `N` for you (since 2026-09-22). | Play refuses a reused `versionCode`; the handler derives `N+1` on every write, so a bump cannot leave `+N` unchanged. An explicit build number that does not increase is refused outright. |
