...
| App | Android + iOS, nothing else | Flutter | React Native; Kotlin Multiplatform |
| App | web only | *decided by the web research pass* | — |
| App | two or more of desktop / mobile / web | Flutter | React Native + React web; Kotlin Multiplatform + Compose Multiplatform |
...
- *React Native is researched, not stubbed.* It started as a stub; it stopped being one when web
  was ticked in. With desktop + mobile + web all in scope, the contest is Flutter (one codebase,
  canvas-rendered web) against React Native + React web (one ecosystem, two codebases,
  native-ish mobile), and direflail's lean toward EAS Build for iOS signing — Expo's service,
  built around React Native — is a thumb on that scale. The cross-family pass has to answer
  which one direflail would rather live in, not which one has the longer feature list.
...
| 3 | React Native | `skills/stack-react-native/SKILL.md` — mobile; with React web for the cross-family case; EAS Build and what choosing it commits you to |
| 4 | Web | `skills/stack-web/SKILL.md` — the web-only row's default and its concern lines; front and back candidates from #4 |
| 5 | Cross-family | The "two or more families" row's default, decided by pitting passes 1–4's findings against Flutter's per-platform reality — Flutter on Linux desktop and on web are the two claims that most need checking live. Lands as sections in `stack-flutter` and in whichever alternative wins a concern line |
...
| 11 | iOS build/sign from Linux | A named cloud option in `stack-ios-native` and `stack-flutter` (Codemagic, GitHub Actions macOS runners, EAS) — today both say "a Mac at hand or a cloud Mac" and name nothing |

Sequence: 1 and 6–10 first (they touch what exists and what is most likely to be used next),
then 2, 3, 4, then 5 (it depends on 2–4), 11 alongside whichever of 6–8 is in hand. Each pass is
